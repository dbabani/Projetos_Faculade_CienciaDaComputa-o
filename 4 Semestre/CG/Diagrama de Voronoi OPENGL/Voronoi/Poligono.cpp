...
void Poligono::obtemLimites(Ponto &Min, Ponto &Max)
{
    ...
    //Sempre que pega os min e max ja gera os envelopes
    envelope.GeraEnvelope(Min,Max);
}
...
void Poligono::imprimeVizinhos(){
    for(int i = 0; i< Vertices.size();i++){
        cout  <<"O vizinho do vertice " << i << " eh: "<< Vertices[i].getVizinho()<< "\n" << endl;
    }
}
void Poligono::setVizinho(int vertice, int vizinho)
{
    Vertices[vertice].vizinho = vizinho;
}
